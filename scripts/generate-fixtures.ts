...
const fuelRows: string[][] = [
  ["Month", "Plant / Section", "Material Desc.", "GRN Ref", "Qty Consumed", "UOM", "Rate (Rs/MT)", "Remarks"],
];
...
for (const m of MONTHS) {
  ...
  fuelRows.push([
    m.label, "DRI Kiln #1 & #2", "NON COKING COAL (G11)", `GRN/${m.key.replace("-", "")}/0114`,
    inr(kilnCoal, 2), "MT", inr(4850), "",
  ]);

  // Rolling mill reheating furnace, furnace oil in kilolitres.
  fuelRows.push([
    m.label, "Rolling Mill", "FURNACE OIL", `GRN/${m.key.replace("-", "")}/0231`,
    inr(jitter(596, 0.11), 2), "KL", inr(42500), "",
  ]);

  // Site diesel for material handling. Small, and often forgotten entirely.
  fuelRows.push([
    m.label, "Material Handling", "HSD (DIESEL)", `GRN/${m.key.replace("-", "")}/0388`,
    inr(jitter(38.5, 0.16), 2), "KL", inr(89000), m.key === "2026-05" ? "incl. DG set running" : "",
  ]);
}
...
fuelRows.push([
  "Jun-26", "DRI Kiln #1 & #2", "NON COKING COAL (G11) - suppl.", "GRN/202606/0119",
  inr(45_000_000), "MT", inr(4850), "revised entry - check UOM",
]);
...
const powerRows: string[][] = [
  ["Billing Month", "Consumer No.", "Supply Source", "Units Drawn (MU)", "Demand (KVA)", "Amount (Rs)", "Section"],
];
for (const m of MONTHS) {
  if (m.key === "2026-04") continue; // DEFECT 4: bill not exported from the portal
  powerRows.push([
    m.label, "CSPDCL/HT/44127", "CSPDCL Grid (HT)",
    jitter(21.3, 0.07).toFixed(2), inr(28500), inr(jitter(16_80_00_000, 0.07)), "Melt Shop (IF/EAF)",
  ]);
  powerRows.push([
    m.label, "CSPDCL/HT/44127", "CSPDCL Grid (HT)",
    jitter(1.87, 0.09).toFixed(2), inr(28500), inr(jitter(1_47_00_000, 0.08)), "Rolling Mill",
  ]);
  powerRows.push([
    m.label, "PPA/SOLAR/RG-22", "Solar PPA (open access)",
    jitter(1.42, 0.22).toFixed(2), "-", inr(jitter(85_00_000, 0.2)), "Melt Shop (IF/EAF)",
  ]);
}
...
for (const m of MONTHS) {
  materialRows.push([
    m.label, "DRI Kiln #1 & #2", "DOLOMITE (CALCINED)", inr(jitter(534, 0.1), 2), "MT", "V-10233", `B${m.key.slice(2).replace("-", "")}`,
  ]);
  materialRows.push([
    m.label, "Melt Shop (IF/EAF)", "GRAPHITE ELECTRODE (UHP 400mm)", inr(jitter(62.4, 0.12), 2), "MT", "V-20871", `B${m.key.slice(2).replace("-", "")}`,
  ]);
  materialRows.push([
    m.label, "Melt Shop (IF/EAF)", "LIME / LIMESTONE FLUX", inr(jitter(781, 0.09), 2), "MT", "V-10233",
...
const prodRows: string[][] = [
  ["Month", "Section", "Product", "HS / CN Code", "Production (MT)", "Despatched to EU (MT)", "Domestic Despatch (MT)", "Internal Transfer (MT)"],
];
for (const m of MONTHS) {
  const dri = jitter(17_500, 0.08);
  prodRows.push([m.label, "DRI Kiln #1 & #2", "SPONGE IRON (DRI)", "7203 10 00", inr(dri, 2), "0", "0", inr(dri, 2)]);

  const billet = jitter(24_600, 0.07);
  const billetEu = billet * 0.18;
  const billetInternal = billet * 0.62;
  prodRows.push([
    m.label, "Melt Shop (IF/EAF)", "MS BILLETS 125x125", "7207 11 00",
    inr(billet, 2), inr(billetEu, 2), inr(billet - billetEu - billetInternal, 2), inr(billetInternal, 2),
  ]);

  const tmt = jitter(15_100, 0.08);
  const tmtEu = tmt * 0.41;
  prodRows.push([
    m.label, "Rolling Mill", "TMT REBAR Fe500D", "7213 10 00",
    inr(tmt, 2), inr(tmtEu, 2), inr(tmt - tmtEu, 2), "0",
  ]);
}
prodRows.push(["Aug-26", "Workshop", "MS FABRICATED SPARES", "8455 90 00", "42.00", "12.00", "30.00", "0"]);
...
const precursorRows: string[][] = [
  ["Month", "Consuming Section", "Material", "CN Code", "Qty Received (MT)", "Supplier", "CBAM Communication Recd?", "Supplier SEE Direct (tCO2e/t)", "Supplier SEE Indirect"],
];
for (const m of MONTHS) {
  precursorRows.push([
    m.label, "Melt Shop (IF/EAF)", "SPONGE IRON (bought out)", "7203 10 00",
    inr(jitter(4_200, 0.15), 2), "Jindal Sponge (Raigarh)", "Yes", "1.038", "0.071",
  ]);
  precursorRows.push([
    m.label, "Melt Shop (IF/EAF)", "SPONGE IRON (bought out)", "7203 10 00",
    inr(jitter(1_950, 0.2), 2), "Maa Ambey Ispat", "No", "-", "-",
  ]);
  precursorRows.push([
    m.label, "Melt Shop (IF/EAF)", "FERRO SILICON 70%", "7202 21 00",
    inr(jitter(148, 0.14), 2), "Balasore Alloys", "Yes", "2.410", "1.580",
  ]);
}
